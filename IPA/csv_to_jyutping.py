    ...
    for char in sentence:
        ...
        try:
            ...
            if jyutping_list and jyutping_list[0]:
                ...
                logger.warning(f"无法为字符 '{char}' 获取粤语拼音")
                failed_words.append(char)
                success = False
                ipa_parts.append(char)  # Fallback to character
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Chinese text in CSV to Jyutping IPA, treating each character as a word."
    )
    ...
    # Check if input CSV contains Mandarin text
    logger.warning(
