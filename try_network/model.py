...
class PosteriorEncoder(nn.Module):
    '''x:[batchsize, in_channels, timesteps] -> z:[batch_size, out_channels, timesteps]'''

    '''x:[batchsize, in_channels, timesteps] -> m:[batch_size, out_channels, timesteps]'''
    '''x:[batchsize, in_channels, timesteps] -> logs:[batch_size, out_channels, timesteps]'''
    '''x:[batchsize, in_channels, timesteps] -> x_mask:[batch_size, 1, timesteps]'''
...
class Decoder(torch.nn.Module):
    def __init__(
        ...
        self.conv=nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1),  # 保持形状
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(32, 1, kernel_size=3, stride=1, padding=1)
        )
        self.reenc = modules.WN(
            hidden_channels,
            kernel_size,
    ...
    def forward(self, x, x_lengths, g=None):
        ...
        x = self.reproj(x) * x_mask
        x = self.conv(x.unsqueeze(1))  # [b, 1, c, t]
        x = x.squeeze(1)  # [b, c, t]
        #x = self.reenc(x, x_mask)
        x = self.repre(x) * x_mask
        return x
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        ...
        gen_mel = self.dec(z, torch.tensor([z.shape[-1]]).to(device=z.device))
        return (
            gen_mel,
            l_length,
            attn,
