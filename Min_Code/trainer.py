...
def create_optimizer(model, config):
    ...
    if hasattr(torch.optim, optimizer_name):
        ...
        print(
            f"⚠️  Optimizer {optimizer_name} does not exist, using default Adam"
        )
    ...
    try:
        optimizer = optimizer_class(
            model.parameters(), lr=lr, **optimizer_params
        )
        print(
            f"✅ Created optimizer: {optimizer_name}, LR: {lr}, Parameters: {optimizer_params}"
        )
        return optimizer
...
def create_scheduler(optimizer, config):
    ...
    if scheduler_name == "NoamLR":
        ...
        print(
            f"✅ Created scheduler: StepLR, step_size: {step_size}, gamma: {gamma}"
        )

    else:
        # Default to exponential decay
        print(
            f"⚠️  Scheduler {scheduler_name} not supported, using default ExponentialLR"
        )
...
class GlowTTSTrainer:
    ...
    def __init__(self, model, config, output_path="./outputs"):
        # Create directories
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.train_loader is None:
            self.train_loader = self.model.get_data_loader(
                config=self.config, is_eval=False
            )
    ...
    def get_eval_dataloader(self) -> DataLoader:
        ...
        if self.eval_loader is None:
            self.eval_loader = self.model.get_data_loader(
                config=self.config, is_eval=True
            )
    ...
    def train_step(self, batch: Dict) -> Tuple[Dict, Dict]:
        ...
        self.model.run_data_dep_init = (
            self.total_steps_done < data_dep_init_steps
        )
        ...
        outputs, loss_dict = self.model.train_step(
            formatted_batch, self.criterion
        )
        ...
        if self.scheduler is not None:
            ...
            if scheduler_name == "NoamLR" or not getattr(
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(train_loader):
            ...
            if not loss_dict:
                print(
                    f" [!] Step {step} skipped (data dependency initialization)"
                )
                continue
    ...
    def eval_epoch(self):
        ...
        for step, batch in enumerate(eval_loader):
            ...
            # Update statistics
            self.keep_avg_eval.update_values(
                {f"avg_{k}": v for k, v in loss_dict.items()}
            )
            ...
            if (step + 1) % self.config.print_step == 0:
                print(
                    f"📈 Evaluation step {step+1}/{len(eval_loader)} | "
                    f"Loss: {loss_dict['loss']:.4f}"
                )
    ...
    def save_checkpoint(self):
        ...
        checkpoint_path = (
            f"{self.output_path}/checkpoint_step_{self.total_steps_done}.pth"
        )
    ...
    def save_best_model(self):
        """Save best model"""
        torch.save(
            self.model.state_dict(), f"{self.output_path}/best_model.pth"
        )
    ...
    def load_checkpoint(self, checkpoint_path, config=None):
        ...
        print(
            f"📊 Restored state: Step {self.total_steps_done}, Epoch {self.epochs_done}, Best loss: {self.best_loss:.4f}"
        )
    ...
    def fit(self):
        ...
        print(
            f"📋 Configuration: {self.config.epochs} epochs, Batch size: {self.config.batch_size}"
        )
