...
class Decoder(torch.nn.Module):
    def __init__(
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1),  # 保持形状
            nn.ReLU(),
    ...
    def forward(self, x,g=None):#remember to add mask
        x_mask = torch.unsqueeze(commons.sequence_mask(torch.tensor([len(x)]).cuda(), x.size(2)), 1).to(
            x.dtype
        )
        x = self.reproj(x) 
        x = self.attention(x)
        ...
        **kwargs
        ...
        if use_sdp:
            ...
            self.dp = DurationPredictor(
                hidden_channels, 256, 3, 0.1, gin_channels=gin_channels
            )
